...
export default function CatalogScreen({ navigation }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    try {
      ...
      console.log('Failed to load catalog:', error);
      setCourses([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const greetName =
    me?.first_name || me?.username || 'there';
          ...
          <View style={styles.headerRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.hiSub}>
                Embark on a personalized security training
                journey.
              </Text>
            </View>
